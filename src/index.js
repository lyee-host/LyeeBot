/**
   ...
   * Recode By LyeeHost
   * Contact Me on wa.me/62895330146373
   * Follow https://github.com/YosokaHostingdev
*/

require('./config')
const { default: YosokaHostingConnect, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
...
async function startYosokaHosting() {
    const YosokaHosting = YosokaHostingConnect({
        ...
        browser: ['YT YosokaHosting','Safari','1.0.0'],
        auth: state
    })
    ...
    // Get Profile Picture Group
       ...
       let wm_YosokaHostingdev = { url : ppgc }
       if (pea[0].announce == true) {
       YosokaHosting.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nGroup telah ditutup oleh admin, Sekarang hanya admin yang dapat mengirim pesan !`, `Group Settings Change Message by YosokaHosting Dev`, wm_YosokaHostingdev, [])
       } else if(pea[0].announce == false) {
       YosokaHosting.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nGroup telah dibuka oleh admin, Sekarang peserta dapat mengirim pesan !`, `Group Settings Change Message by YosokaHosting Dev`, wm_YosokaHostingdev, [])
       } else if (pea[0].restrict == true) {
       YosokaHosting.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nInfo group telah dibatasi, Sekarang hanya admin yang dapat mengedit info group !`, `Group Settings Change Message by YosokaHosting Dev`, wm_YosokaHostingdev, [])
       } else if (pea[0].restrict == false) {
       YosokaHosting.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nInfo group telah dibuka, Sekarang peserta dapat mengedit info group !`, `Group Settings Change Message by YosokaHosting Dev`, wm_YosokaHostingdev, [])
       } else {
       YosokaHosting.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nGroup Subject telah diganti menjadi *${pea[0].subject}*`, `Group Settings Change Message by YosokaHosting Dev`, wm_YosokaHostingdev, [])
     }
    })
    ...
    YosokaHosting.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                let nyoutube = ('© YosokaHosting\nYoutube/Sc :\nhttps://youtube.com/c/YosokaHostingdev')
    ...
    /** Resize Image
      ...
      */
      YosokaHosting.reSize = async (image, width, height) => {
    ...
    /** Send List Messaage
      ...
      */
       ...
       * @param {*} options
       */
      YosokaHosting.send5ButLoc = async (jid , text = '' , footer = '', lok, but = [], options = {}) =>{
      let bb = await YosokaHosting.reSize(lok, 300, 300)
      YosokaHosting.sendMessage(jid, { location: { jpegThumbnail: bb }, caption: text, footer: footer, templateButtons: but, ...options })
      }
